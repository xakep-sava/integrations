...
const events = ['assistant.intent.action.MAIN', 'assistant.intent.action.TEXT', 'assistant.intent.action.PERMISSION']
...
export class Adapter {
    ...
    return Observable.merge(...fromEvents)
      .switchMap(value => {
        return Observable.of(value)
          .mergeMap((normalized: actionsSdk.ActionsSdkAssistant) => this.parser.parse(normalized))
  ...
  public send(data: object): Promise<object | Error> {
    ...
    return schemas(data, 'send').then(() => {
      let ssml = false
      const content: any = R.path(['object', 'content'], data) || R.path(['object', 'name'], data)
      ...
      if (objectType === 'Note') {
        return this.sendMessage(ssml, content, noInputs).then(() => ({ type: 'sent', serviceID: this.serviceId() }))
      }
